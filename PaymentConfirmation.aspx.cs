...
public partial class PaymentConfirmatio : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.Form.Count > 0 && !string.IsNullOrEmpty(Request.Form[0]))
        {
            ...
            string sql = "UPDATE dbo.association SET atrn = '" + lblAtrn.Text + "', Transaction_Date='" + Convert.ToDateTime(lblTransDate.Text).ToString("dd/MM/yyyy") + "' WHERE order_id = '" + lblOrderID.Text + "'";
            ...
            try
            {
                ...
                //if (i == 0)
                //{
                sql = "UPDATE dbo.individual SET atrn = '" + lblAtrn.Text + "', Transaction_Date='" + Convert.ToDateTime(lblTransDate.Text).ToString("dd/MM/yyyy") + "' WHERE order_id = '" + lblOrderID.Text + "'";
